window.onload = function() {
    ...
    */
	...
	Ships.prototype.showShip = function() {
		...
		div.style.cssText = 'left:' + (this.y0 * player.shipSize) + 'px; top:' + (this.x0 * player.shipSize) + 'px;';
	...
	Instance.prototype.onMouseDown = function(e) {
		// если нажатие не на левую кнопку прекращаем работу функции
	...
	Instance.prototype.getCoordsClone = function(decks) {
		...
		coords.top	= (top < 0) ? 0 : (bottom > user.fieldSide) ? user.fieldSide - user.shipSize : top;
		...
		coords.left = (left < 0) ? 0 : (right > user.fieldSide) ? user.fieldSide - user.shipSize * decks : left;
	...
	Instance.prototype.rotationShip = function(e) {
		...
		for (var i = 0, length = user.squadron.length; i < length; i++) {
			...
			if (data.shipname == id && data.decks != 1) {
				...
				if (result === false) {
					// если новые координаты валидацию не прошли, возвращаем коэфициэнтам
					// предыдущие значения
					var kx	= (kx == 0) ? 1 : 0,
						ky	= (ky == 0) ? 1 : 0;
	...
	Instance.prototype.cleanShip = function(el) {
		...
		var coords = el.getBoundingClientRect(),
			x = Math.round((coords.top - user.fieldX) / user.shipSize),
			y = Math.round((coords.left - user.fieldY) / user.shipSize),
			data, k;
	...
	var Controller = (function() {
		...
		var battle = {
			...
			shoot: function(e) {
				...
				switch(val) {
					...
					case 0:
						...
						text = (player === user) ? 'Вы промахнулись. Стреляет компьютер.' : 'Компьютер промахнулся. Ваш выстрел ' + playerName;
					...
					case 1:
						...
						text = (player === user) ? 'Поздравляем! Вы попали. Ваш выстрел ' + playerName : 'Компьютер попал в ваш корабль. Выстрел компьютера';
						self.showServiseText(text);
						...
						if (enemy.squadron.length == 0) {
							...
							// победа компьютера
								...
								for (var i = 0, length = comp.squadron.length; i < length; i++) {
									...
									div.className = 'ship ' + classname + dir;
									div.style.cssText = 'left:' + (comp.squadron[i].y0 * comp.shipSize) + 'px; top:' + (comp.squadron[i].x0 * comp.shipSize) + 'px;';
					...
					case 2:
						...
						[].forEach.call(icons, function(el) {
							...
							var x = el.style.top.slice(0, -2) / comp.shipSize,
								y = el.style.left.slice(0, -2) / comp.shipSize;
			...
			showIcons: function(enemy, coords, iconClass) {
				...
				div.style.cssText = 'left:' + (coords.y * enemy.shipSize) + 'px; top:' + (coords.x * enemy.shipSize) + 'px;';
			...
			setShootMatrixAround: function() {
				...
				// координата правильн, если значение массива не равно или 2 (гарантированно пустая
				...
				for (var i = comp.shootMatrixAround.length - 1; i >= 0; i--) {
					...
					// проверяем правильность этих координат и если они не валидны - удаляем их из массива
			...
			transformCoordinates: function(e, instance) {
				...
				obj.x = Math.trunc((e.pageY - instance.fieldX) / instance.shipSize),
				obj.y = Math.trunc((e.pageX - instance.fieldY) / instance.shipSize);
				return obj;
			},
	...
	function getCoords(el) {
		...
		var coords = el.getBoundingClientRect();
		// пересчитаем координаты относительно документа
		return {
